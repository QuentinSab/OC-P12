...
class ClientView:
    ...
    def show_client_creation_error(self):
        print("\nUne erreur s'est produite lors de l'ajout du client.")
        Utils.temporisation()

    def show_client_creation_success(self):
    ...
    def show_client_not_found(self):
        print("\nClient introuvable.")
        Utils.temporisation()
    ...
    def show_client_modification_success(self):
        print("\nClient modifié avec succès.")
        Utils.temporisation()

    def client_options(self, user_session):
        Utils.clear()
        if user_session.has_permission("can_read_client"):
            print("1 : Voir la liste des clients")

        if user_session.has_permission("can_read_client"):
            print("2 : Voir les détails d'un client")

        if user_session.has_permission("can_create_client"):
            print("3 : Ajouter un client")

        if user_session.has_permission("can_modify_client"):
